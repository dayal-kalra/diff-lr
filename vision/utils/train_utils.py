...
class TrainState(struct.PyTreeNode):
    ...
    def update_params(self, *, new_params, **kwargs):
        ...
        return self.replace(step = self.step + 1, params = new_params, opt_state = self.opt_state, **kwargs,)
...
@partial(jax.jit, static_argnums = 2)
def train_step_minimal(state: TrainState, batch: Tuple, loss_function):
    "Compute gradients, loss and accuracy for a single batch"
    x, y = batch
...
@partial(jax.jit, static_argnums = 2)
def train_step_l2(state: TrainState, batch: Tuple, loss_function, l2_lambda: float = 0.0):
    "Compute gradients, loss and accuracy for a single batch"
    x, y = batch
    ...
    def loss_fn(params):
        ...
        logits = state.apply_fn({'params': params}, x)
        base_loss = loss_function(logits, y)
        flat_params, _ = jax.flatten_util.ravel_pytree(params)
        l2_loss = 0.5 * l2_lambda * jnp.sum(flat_params ** 2)
...
def hessian_trace_frobenius_step(state: TrainState, batch: Tuple, loss_function, n_hutchinson = 30, key = None):
    """
    Description: Estimate the trace and Frobenius norm of the Hessian using Hutchinson's trick
    
    Mathematical background:
        - Trace: tr(H) = E[v^T H v]
        - Frobenius norm squared: ||H||_F^2 = tr(H^T H) = E[||H v||^2]
    ...
    Inputs:
        ...
        -- key: JAX PRNG key for random vector generation
    Returns:
        -- trace_estimate: estimated trace of the Hessian
        -- frobenius_estimate: estimated Frobenius norm of the Hessian
    """
    x, y = batch
    ...
    def body_fun(i, acc):
        """Accumulate both trace and Frobenius norm estimates"""
        trace_sum, frob_sum = acc
        
        # Generate random vector
        subkey = jax.random.fold_in(key, i)
        v = jax.random.rademacher(subkey, (param_dim,), dtype=flat_params.dtype)
        
        # Compute Hessian-vector product
        hv = body_hvp(v)
        
        # Trace: v^T H v
        trace_contrib = jnp.dot(v, hv)
        
        # Frobenius norm squared: ||H v||^2
        frob_contrib = jnp.dot(hv, hv)
        
        return (trace_sum + trace_contrib, frob_sum + frob_contrib)
    
    # Accumulate both estimates
    trace_sum, frob_sum = jax.lax.fori_loop(0, n_hutchinson, body_fun, (0.0, 0.0))
    
    trace_estimate = trace_sum / n_hutchinson
    frobenius_sq_estimate = frob_sum / n_hutchinson  # Take sqrt to get ||H||_F from ||H||_F^2

    return trace_estimate, frobenius_sq_estimate
