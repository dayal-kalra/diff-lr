...
def train_and_evaluate(cfg: argparse.ArgumentParser, train_ds: Tuple, test_ds: Tuple):
    ...
    for step in range(cfg.num_steps):  
        ...
        if step < cfg.warmup_steps:
            lr_step = schedules_utils.polynomial_warmup(
                step = step,
                init_value = cfg.lr_init,
                end_value = cfg.lr_peak,
                num_steps = cfg.warmup_steps,
                exponent = cfg.warmup_exponent
            )
        else:
            # decay
            lr_step = cfg.decay_schedule(
                step = step - cfg.warmup_steps,
                init_value = cfg.lr_peak,
                end_value = cfg.lr_min,
                num_steps = cfg.num_steps - cfg.warmup_steps,
                exponent = cfg.decay_exponent
            )
        ...
        # train for one step
        state, loss_step = train_utils.train_step_minimal(state, batch, cfg.loss_fn)
        # NOTE: debugging purpose only
        # lr_obs = schedules_utilsextract_learning_rate(state.opt_state)
        # print(f"Step: {state.step}, Learning rate: {lr_step:0.4f}, Loss: {loss_step:0.4f}")
...
parser.add_argument('--save_dir', type = str, default = 'cnn_results/mup')
parser.add_argument('--save_ckpt', type = lambda x: False if x.lower() == 'false' else True)
parser.add_argument('--ckpt_dir', type = str, default = 'checkpoints/mup')
...
divergence, train_results, eval_results, num_params = train_and_evaluate(cfg, (x_train, y_train), (x_test, y_test))
